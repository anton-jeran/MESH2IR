...
def compute_discriminator_loss(netD, real_RIRs, fake_RIRs,
    ...
    # print("real_ec shape ",real_ec.shape)
    real_features = nn.parallel.data_parallel(netD, (real_ec), gpus)
    fake_features = nn.parallel.data_parallel(netD, (fake_ec), gpus)
    # real pairs
...
def save_RIR_results_eval(data_RIR, fake,txt_embedding,path, RIR_dir):
    num = len(data_RIR)
    if(not os.path.exists(RIR_dir+"/real")):
        os.mkdir(RIR_dir+"/real")
        os.mkdir(RIR_dir+"/fake")
    if data_RIR is not None:
        for i in range(num):
            # #print("came 1")
            path_loc = str(path[i])
            mesh_name=path_loc[0:path_loc.find("/")]
            RIR_name = str(txt_embedding[i][0]) + "_" +str(txt_embedding[i][1]) + "_" + str(txt_embedding[i][2]) + "_" + str(txt_embedding[i][3]) + "_" + str(txt_embedding[i][4]) + "_" + str(txt_embedding[i][5]) + "_" +  mesh_name
            print("RIR name  ", RIR_name)
            real_RIR_path = RIR_dir+"/real/sample"+"_"+RIR_name+".wav" 
            fake_RIR_path = RIR_dir+"/fake/sample"+"_"+RIR_name+".wav"
            # print("path  ",real_RIR_path)
            fs =16000

            real_IR = np.array(data_RIR[i].to("cpu").detach())
            fake_IR = np.array(fake[i].to("cpu").detach())
            real_IR_only = real_IR[:,0:(4096-128)]
            real_energy = np.median(real_IR[:,(4096-128):4096])*10
            real_IR = real_IR_only*real_energy
            fake_IR_only = fake_IR[:,0:(4096-128)]
            fake_energy = np.median(fake_IR[:,(4096-128):4096])*10
            fake_IR = fake_IR_only*fake_energy

            energy_factor = 1 #np.max(real_IR)/np.max(fake_IR)
...
def convert_IR2EC_batch(rir, filters, filter_length):
    ...
    rir = rir[:,:,0:3968]
    subband_ECs = cp.zeros((rir.shape[0],filters.shape[3],rir.shape[2]))
    for i in range(filters.shape[3]):
        subband_ir = fftconvolve(rir, filters[:,:,:, i])
        subband_ir = subband_ir[:,:,(filter_length - 1):]
        squared = cp.square(subband_ir[:,:,:rir.shape[2]])
        subband_output= cp.cumsum(squared[:,:,::-1],axis=2)[:,:,::-1].reshape([rir.shape[0],rir.shape[2]])
        subband_ECs[:, i,:] = subband_output
    # print("subband_ECs shape 1",subband_ECs.shape)
    # print("subband_ECs shape 2",subband_ECs.shape)
    subband_ECs = cp.concatenate((subband_ECs,subband_ECs[:,:,3840:3968]),axis=2)
    subband_ECs = torch.tensor(subband_ECs,dtype=torch.float,device='cuda')
    return subband_ECs
